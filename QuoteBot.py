...
async def add(ctx, *args: str):
    ...
    params = ["", "", ""]
    #//////////
    #TODO: Make sure that the comment only has 3 parameters
    #//////////
    for arg in args:
        if paramCounter > 2:
            break 
            ...
            if(arg.endswith('-')):
                lastWord = arg.rstrip('-')
                params[paramCounter] +=  (lastWord + " ")
                paramCounter += 1
                continue
            params[paramCounter] +=  (arg + " ")

    message = "**New quote** {} \n *\"{}\"* \n- {} \n added by {}"
    #//////////
    #TODO: Add the quote in the sqlite table
    #//////////
    await ctx.send(message.format(params[0], params[1], params[2], ctx.author.name))
