...
async def rando(ctx):
    ...
    if db[0]:
        ...
        result = c.fetchall()
        if not result:
            await ctx.send("**Oups** it seems like the cookie jar is empty")
        else:
            quoteCount = 0
            for row in result:
                quoteCount += 1
            randRow = result[random.randrange(quoteCount - 1)]
...
async def show(ctx, title: str):
    ...
    if db[0]:
        ...
        result = c.fetchone()
        if result is None:
            await ctx.send("**Oups** it seems there's a typo or this quote doesn't exist")
        else:
